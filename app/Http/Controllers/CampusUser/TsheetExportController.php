...
class TsheetExportController extends Controller
{
    /**
     * Preview TSHEET export for Planning Coordinator (HTML only)
     * 
    ...
    public function preview(Request $request)
    {
        ...
        return view('campus-user.reports.tsheet-preview', [
    ...
    public function export(Request $request)
    {
        ...
        if ($submissions->count() > 0) {
            $campusName = $submissions->first()->campus ?? null;
            $templateCode = $submissions->first()->template_code ?? null;
        }
        ...
        $campusCode = strtoupper(str_replace(' ', '_', $campusName ?? 'CAMPUS'));
        $templateCodeClean = strtoupper($templateCode ?? 'ALL');
        $filename = $campusCode . '_TSHEET_' . $templateCodeClean . '_' . now()->format('Y-m-d') . '.xlsx';
