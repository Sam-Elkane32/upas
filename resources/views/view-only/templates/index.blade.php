        ...
        <div class="flex justify-between items-center">
            <div>
                ...
                <p class="text-sm text-gray-600 mt-1">
                    Preview published templates across all campuses
                </p>
            </div>
            <div>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-6 bg-blue-50 border border-blue-200 text-blue-800 px-4 py-3 rounded">
                <div class="flex items-center">
                    ...
                    <p class="font-medium">You are viewing in Read-Only mode. Only published templates are displayed.</p>
                </div>
            </div>

            <!-- Filters -->
            <div class="bg-white shadow-sm rounded-lg mb-6 p-4">
                <form method="GET" action="{{ route('view-only.templates.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div>
                    ...
                    <div>
                        ...
                        <select name="sort_by" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            ...
                            <option value="template_code" {{ request('sort_by') == 'template_code' ? 'selected' : '' }}>Template Code</option>
                            <option value="sg_code" {{ request('sort_by') == 'sg_code' ? 'selected' : '' }}>SG Code</option>
                        </select>
                    </div>
                    ...
                    <div class="md:col-span-4 flex justify-end space-x-2">
                        ...
                        <a href="{{ route('view-only.templates.index') }}" class="px-4 py-2 bg-gray-600 text-white rounded-md hover:bg-gray-700">
                            Clear
                        </a>
                    </div>
                </form>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $template->template_code }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $template->campus_code }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $template->sg_code }}</td>
                                <td class="px-6 py-4 text-sm text-gray-900">{{ Str::limit($template->kra_title, 50) }}</td>
                                <td class="px-6 py-4 text-sm text-gray-900">{{ Str::limit($template->kpi_title, 50) }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $template->creator->name ?? 'N/A' }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm">
                                    <a href="{{ route('view-only.templates.show', $template->id) }}" class="text-blue-600 hover:text-blue-900">View</a>
                                </td>
                            </tr>
                            @empty
                            <tr>
                                <td colspan="7" class="px-6 py-4 text-center text-sm text-gray-500">No published templates found.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
