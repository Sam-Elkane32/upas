                                            ...
                                            @php
                                                ...
                                                foreach ($tableHeaders as $h) {
                                                    ...
                                                    if (str_contains($n, 'evidence') && str_contains($n, 'verified') && str_contains($n, 'qa')) {
                                                        $evidenceColKey = $h;
                                                        $resultHeaderKeys[$h] = true;
                                                    }
                                                    $hl = preg_replace('/\.+$/', '', $n);
                                                    if ($hl === 'no') $resultHeaderKeys[$h] = true;
                                                }
                                            @endphp
                                            @if(count($submission->table_data) > 0)
                                                @foreach($tableHeaders as $header)
                                                    <th scope="col" class="px-4 py-3 text-left text-xs font-bold text-gray-700 uppercase tracking-wider whitespace-nowrap">
                                        ...
                                        @foreach($submission->table_data as $index => $row)
                                            ...
                                            <tr class="transition-colors duration-150 {{ $isSummaryRow ? 'bg-blue-100 hover:bg-blue-100' : ($index % 2 === 0 ? 'bg-white hover:bg-blue-50' : 'bg-gray-50 hover:bg-blue-50') }}">
                                                @foreach($tableHeaders as $header)
                                                    ...
                                                    @endphp
                                                    <td class="px-4 py-4 text-sm {{ $cellBlue }}">
                                                        @if($isEvidenceCol && !$isSummaryRow)
                                                            <select name="evidence_qa[{{ $index }}]" class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                                        ...
                                                        @elseif($value !== '' && filter_var($value, FILTER_VALIDATE_URL))
                                                            <a href="{{ $value }}" target="_blank" class="{{ $isSummaryRow ? 'text-blue-700' : 'text-indigo-600 hover:text-indigo-900' }} underline break-all">
                                                                <svg class="w-4 h-4 inline mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"></path>
                                                                </svg>
                                                                Open Link
                                                            </a>
                                                        @elseif(strlen($displayValue) > 100 && $displayValue !== '-')
                                                            <div class="max-w-xs">
                                                                <span class="block truncate" title="{{ $displayValue }}">{{ $displayValue }}</span>
                                                            </div>
                                                        @else
