<x-app-layout>
    ...
    <div class="pt-2 pb-4">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white rounded-xl shadow-lg border border-gray-200 p-6 mb-6">
                <div class="flex justify-between items-center">
                    ...
                    <div class="flex space-x-2">
                        ...
                        <a href="{{ route('super-admin.templates.index', ['tab' => 'forms']) }}" class="inline-flex items-center px-4 py-2 bg-gray-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                            Back To Forms
                        </a>
                    </div>
                </div>
            ...
            <form action="{{ route('forms.update', $form->id) }}" method="POST" id="edit-form-form">
                @csrf
                @method('PUT')
                    ...
                    <div class="p-6">
                        ...
                        <div>
                            ...
                            <select id="division" name="division" required 
                                ...
                                <option value="OP" {{ old('division', $form->division) == 'OP' ? 'selected' : '' }}>Office of the President (OP)</option>
                                <option value="OVPAFM" {{ old('division', $form->division) == 'OVPAFM' ? 'selected' : '' }}>Office of the Vice President for Administration and Finance Management (OVPAFM)</option>
                                <option value="OVPASS" {{ old('division', $form->division) == 'OVPASS' ? 'selected' : '' }}>Office of the Vice President for Academic and Student Services (OVPASS)</option>
                                <option value="OVPREI" {{ old('division', $form->division) == 'OVPREI' ? 'selected' : '' }}>Office of the Vice President for Research, Extension & Innovation (OVPREI)</option>
                                <option value="OVPQA" {{ old('division', $form->division) == 'OVPQA' ? 'selected' : '' }}>Office of the Vice President for Quality Assurance (OVPQA)</option>
                                <option value="OVPLIA" {{ old('division', $form->division) == 'OVPLIA' ? 'selected' : '' }}>Office of the Vice President for Local & International Affairs (OVPLIA)</option>
                            </select>
                            @error('division')
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
                    <div class="p-6">
                        ...
                        <div id="kra-container">
                            @if(isset($kraKpiData) && is_array($kraKpiData) && count($kraKpiData) > 0)
                                @foreach($kraKpiData as $kraIndex => $kraData)
                                    <div class="kra-item mb-6 p-4 border-2 border-gray-300 rounded-lg bg-gray-50" data-kra-index="{{ $kraIndex }}">
                                        <div class="flex justify-between items-center mb-4">
                                            @php
                                                $sgNumber = str_replace('SG', '', $form->sg_code ?? 'SG1');
                                                $kraNumber = $sgNumber . '.' . ($kraIndex + 1);
                                            @endphp
                                            <h4 class="text-md font-semibold text-gray-800 kra-title">KRA {{ $kraNumber }}</h4>
                                            <button type="button" class="remove-kra text-red-600 hover:text-red-800" style="{{ count($kraKpiData) > 1 ? '' : 'display: none;' }}">
                                        ...
                                        <div class="mb-4">
                                            ...
                                            <input type="text" name="kra_titles[]" required 
                                                   ...
                                                   value="{{ old("kra_titles.{$kraIndex}", $kraData['kra_title'] ?? '') }}">
                                            @error("kra_titles.{$kraIndex}")
                                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                            @enderror
                                        </div>
                                        ...
                                        <div class="mb-4">
                                            ...
                                            <div class="kpi-container" data-kra-index="{{ $kraIndex }}">
                                                @if(isset($kraData['kpis']) && count($kraData['kpis']) > 0)
                                                    @php
                                                        // Calculate global KPI index
                                                        $globalKpiIndex = 0;
                                                        foreach($kraKpiData as $idx => $kra) {
                                                            if ($idx < $kraIndex) {
                                                                $globalKpiIndex += count($kra['kpis'] ?? []);
                                                            }
                                                        }
                                                    @endphp
                                                    @foreach($kraData['kpis'] as $kpiIndex => $kpi)
                                                        @php
                                                            $currentGlobalKpiIndex = $globalKpiIndex + $kpiIndex;
                                                        @endphp
                                                        <div class="kpi-item mb-3 p-3 border border-gray-200 rounded-md bg-white">
                                                            {{-- KPI Title: Full-width row for equalization and responsiveness --}}
                                                            <div class="mb-4">
                                                                <label class="block text-sm font-medium text-gray-700 mb-2">
                                                                    KPI Title
                                                                </label>
                                                            ...
                                                            <div class="grid grid-cols-12 gap-4 items-end mb-3">
                                                                <div class="col-span-12 sm:col-span-2 min-w-0">
                                                                    ...
                                                                    <input type="text" name="kpi_numbers[{{ $kraIndex }}][]" required 
                                                                           ...
                                                                           placeholder="e.g., 1"
                                                                           value="{{ old("kpi_numbers.{$kraIndex}.{$kpiIndex}", $kpi['number'] ?? '') }}">
                                                                </div>
                                                                <div class="col-span-6 sm:col-span-5 min-w-0">
                                                                    ...
                                                                    <div class="responsible-unit-multi mt-1 relative">
                                                                        <input type="hidden" name="responsible_units[{{ $kraIndex }}][]" class="responsible-unit-value" value="{{ old("responsible_units.{$kraIndex}.{$kpiIndex}", $kpi['responsible_unit'] ?? '') }}" required>
                                                                ...
                                                                <div class="col-span-6 sm:col-span-5 min-w-0">
                                                                    ...
                                                                    @php
                                                                        $kpiLevels = old("kpi_levels.{$kraIndex}.{$kpiIndex}", $kpi['level'] ?? []);
                                                                        if (!is_array($kpiLevels)) {
                                                                            $kpiLevels = $kpiLevels ? [$kpiLevels] : [];
                                                                        }
                                                                        // Determine selected value for dropdown
                                                                        $selectedLevel = '';
                                                                        if (in_array('CL', $kpiLevels) && in_array('UL', $kpiLevels)) {
                                                                            $selectedLevel = 'CL_UL';
                                                                        } elseif (in_array('CL', $kpiLevels)) {
                                                                            $selectedLevel = 'CL';
                                                                        } elseif (in_array('UL', $kpiLevels)) {
                                                                            $selectedLevel = 'UL';
                                                                        }
                                                                    @endphp
                                                                    <select name="kpi_levels[{{ $kraIndex }}][{{ $kpiIndex }}]" required
                                                                        ...
                                                                        <option value="CL" {{ $selectedLevel == 'CL' ? 'selected' : '' }}>Campus Level (CL)</option>
                                                                        <option value="UL" {{ $selectedLevel == 'UL' ? 'selected' : '' }}>University Level (UL)</option>
                                                                        <option value="CL_UL" {{ $selectedLevel == 'CL_UL' ? 'selected' : '' }}>Both (CL / UL)</option>
                                                                    </select>
                                                                </div>
                                                            </div>
                                                            <div class="border-t border-gray-200 pt-3">
                                                                @php 
                                                                    $kpiIsPct = !empty($kpi['is_percentage']); 
                                                                    $totalMode = $kpi['total_mode'] ?? 'average';
                                                                @endphp
                                                                <div class="flex items-center justify-between mb-2">
                                                                    <label class="block text-sm font-medium text-gray-700">Target Values</label>
                                                                    <label class="inline-flex items-center text-sm text-gray-600">
                                                                        <input type="checkbox" name="is_percentage_{{ $currentGlobalKpiIndex }}" value="1" class="kpi-is-percentage rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" data-kpi-index="{{ $currentGlobalKpiIndex }}" {{ $kpiIsPct ? 'checked' : '' }}>
                                                                        <span class="ml-2">Values in percentage (%)</span>
                                                                    </label>
                                                                </div>
                                                                <div class="flex items-center gap-2 mb-2">
                                                                    <span class="text-xs text-gray-600">Total:</span>
                                                                    <input type="hidden" name="target_total_mode_{{ $currentGlobalKpiIndex }}" value="{{ $totalMode }}" class="kpi-total-mode-input">
                                                                    <button type="button" class="kpi-total-mode-btn px-2 py-1 text-xs font-medium rounded border {{ $totalMode === 'sum' ? 'border-indigo-600 bg-indigo-600 text-white' : 'border-gray-300 bg-white text-gray-700' }}" data-mode="sum">Sum</button>
                                                                    <button type="button" class="kpi-total-mode-btn px-2 py-1 text-xs font-medium rounded border {{ $totalMode === 'average' ? 'border-indigo-600 bg-indigo-600 text-white' : 'border-gray-300 bg-white text-gray-700' }}" data-mode="average">Average</button>
                                                                </div>
                                                                <div class="grid grid-cols-2 md:grid-cols-5 gap-3 target-values-grid">
                                                                    <div>
                                                                        <label class="block text-xs font-medium text-gray-600 mb-1">Q1 <span class="target-pct-suffix text-gray-500" style="{{ $kpiIsPct ? '' : 'display:none' }}">(%)</span></label>
                                                                        <input type="number" name="target_q1_{{ $currentGlobalKpiIndex }}" min="0" step="0.01" max="100"
                                                                               ...
                                                                               data-kpi-index="{{ $currentGlobalKpiIndex }}" data-quarter="1"
                                                                               value="{{ old("target_q1_{$currentGlobalKpiIndex}", $kpi['target_q1'] ?? '') }}"
                                                                               placeholder="0">
                                                                    </div>
                                                                    <div>
                                                                        <label class="block text-xs font-medium text-gray-600 mb-1">Q2 <span class="target-pct-suffix text-gray-500" style="{{ $kpiIsPct ? '' : 'display:none' }}">(%)</span></label>
                                                                        <input type="number" name="target_q2_{{ $currentGlobalKpiIndex }}" min="0" step="0.01" max="100"
                                                                               class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm kpi-target-input"
                                                                               data-kpi-index="{{ $currentGlobalKpiIndex }}" data-quarter="2"
                                                                               value="{{ old("target_q2_{$currentGlobalKpiIndex}", $kpi['target_q2'] ?? '') }}"
                                                                               placeholder="0">
                                                                    </div>
                                                                    <div>
                                                                        <label class="block text-xs font-medium text-gray-600 mb-1">Q3 <span class="target-pct-suffix text-gray-500" style="{{ $kpiIsPct ? '' : 'display:none' }}">(%)</span></label>
                                                                        <input type="number" name="target_q3_{{ $currentGlobalKpiIndex }}" min="0" step="0.01" max="100"
                                                                               class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm kpi-target-input"
                                                                               data-kpi-index="{{ $currentGlobalKpiIndex }}" data-quarter="3"
                                                                               value="{{ old("target_q3_{$currentGlobalKpiIndex}", $kpi['target_q3'] ?? '') }}"
                                                                               placeholder="0">
                                                                    </div>
                                                                    <div>
                                                                        <label class="block text-xs font-medium text-gray-600 mb-1">Q4 <span class="target-pct-suffix text-gray-500" style="{{ $kpiIsPct ? '' : 'display:none' }}">(%)</span></label>
                                                                        <input type="number" name="target_q4_{{ $currentGlobalKpiIndex }}" min="0" step="0.01" max="100"
                                                                               class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm kpi-target-input"
                                                                               data-kpi-index="{{ $currentGlobalKpiIndex }}" data-quarter="4"
                                                                               value="{{ old("target_q4_{$currentGlobalKpiIndex}", $kpi['target_q4'] ?? '') }}"
                                                                               placeholder="0">
                                                                    </div>
                                                                    <div>
                                                                        <label class="block text-xs font-medium text-gray-600 mb-1">Total <span class="target-pct-suffix text-gray-500" style="{{ $kpiIsPct ? '' : 'display:none' }}">(%)</span></label>
                                                                        <input type="number" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm bg-gray-100 sm:text-sm kpi-target-total-{{ $currentGlobalKpiIndex }}"
                                                                               readonly
                                                                               value="{{ old("target_total_{$currentGlobalKpiIndex}", ($kpi['target_total'] ?? (($kpi['target_q1'] ?? 0) + ($kpi['target_q2'] ?? 0) + ($kpi['target_q3'] ?? 0) + ($kpi['target_q4'] ?? 0)))) }}"
                                                                               placeholder="0.00">
                                                                    </div>
                                                                </div>
                                                            </div>
                                                ...
                                                @else
                                                    @php
                                                        // Calculate global KPI index for empty KRA
                                                        $globalKpiIndexEmpty = 0;
                                                        foreach($kraKpiData as $idx => $kra) {
                                                            if ($idx < $kraIndex) {
                                                                $globalKpiIndexEmpty += count($kra['kpis'] ?? []);
                                                            }
                                                        }
                                                    @endphp
                                                    <div class="kpi-item mb-3 p-3 border border-gray-200 rounded-md bg-white">
                                                        <div class="mb-4">
                                                            ...
                                                            <textarea name="kpi_titles[{{ $kraIndex }}][]" required rows="6"
                                                                      class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm min-h-[120px] resize-y"
                                                                      placeholder="e.g., Number of reviewed, enhanced, and CHED-approved curriculum&#10;a. Industry-driven&#10;b. SDGs&#10;c. 21st century skills">{{ old("kpi_titles.{$kraIndex}.0") }}</textarea>
                                                        </div>
                                                        <div class="grid grid-cols-12 gap-4 items-end mb-3">
                                                            <div class="col-span-12 sm:col-span-2 min-w-0">
                                                                ...
                                                                <input type="text" name="kpi_numbers[{{ $kraIndex }}][]" required 
                                                                       class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm font-mono kpi-number-input"
                                                                       placeholder="e.g., 1"
                                                                       value="{{ old("kpi_numbers.{$kraIndex}.0", '1') }}">
                                                            </div>
                                                            <div class="col-span-6 sm:col-span-5 min-w-0">
                                                                <label class="block text-sm font-medium text-gray-700 mb-2">Responsible Unit <span class="text-red-500">*</span></label>
                                                                <div class="responsible-unit-multi mt-1 relative">
                                                                    <input type="hidden" name="responsible_units[{{ $kraIndex }}][]" class="responsible-unit-value" value="{{ old("responsible_units.{$kraIndex}.0") }}" required>
                                                                    <button type="button" class="responsible-unit-trigger w-full flex items-center justify-between gap-2 px-3 py-2 border border-gray-300 rounded-md shadow-sm bg-white text-left text-sm focus:ring-indigo-500 focus:border-indigo-500">
                                                                        <span class="responsible-unit-trigger-text text-gray-500 truncate">Select Responsible Unit(s)...</span>
                                                                        <svg class="responsible-unit-chevron w-4 h-4 text-gray-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                                                                    </button>
                                                            ...
                                                            <div class="col-span-6 sm:col-span-5 min-w-0">
                                                                ...
                                                                <select name="kpi_levels[{{ $kraIndex }}][0]" required
                                                                        class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm kpi-level-select">
                                                                    <option value="">Select Level</option>
                                                                    <option value="CL" {{ old("kpi_levels.{$kraIndex}.0") == 'CL' ? 'selected' : '' }}>Campus Level (CL)</option>
                                                                    <option value="UL" {{ old("kpi_levels.{$kraIndex}.0") == 'UL' ? 'selected' : '' }}>University Level (UL)</option>
                                                                    <option value="CL_UL" {{ old("kpi_levels.{$kraIndex}.0") == 'CL_UL' ? 'selected' : '' }}>Both (CL / UL)</option>
                                                                </select>
                                                            </div>
                                                        </div>
                                                        <div class="border-t border-gray-200 pt-3">
                                                            <div class="flex items-center justify-between mb-2">
                                                                <label class="block text-sm font-medium text-gray-700">Target Values</label>
                                                                <label class="inline-flex items-center text-sm text-gray-600">
                                                                    <input type="checkbox" name="is_percentage_{{ $globalKpiIndexEmpty }}" value="1" class="kpi-is-percentage rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" data-kpi-index="{{ $globalKpiIndexEmpty }}">
                                                                    <span class="ml-2">Values in percentage (%)</span>
                                                                </label>
                                                            </div>
                                                            <div class="flex items-center gap-2 mb-2">
                                                                <span class="text-xs text-gray-600">Total:</span>
                                                                <input type="hidden" name="target_total_mode_{{ $globalKpiIndexEmpty }}" value="average" class="kpi-total-mode-input">
                                                                <button type="button" class="kpi-total-mode-btn px-2 py-1 text-xs font-medium rounded border border-gray-300 bg-white text-gray-700" data-mode="sum">Sum</button>
                                                            ...
                                                            <div class="grid grid-cols-2 md:grid-cols-5 gap-3 target-values-grid">
                                                                <div>
                                                                    ...
                                                                    <input type="number" name="target_q1_{{ $globalKpiIndexEmpty }}" min="0" step="0.01" max="100"
                                                                           class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm kpi-target-input"
                                                                           data-kpi-index="{{ $globalKpiIndexEmpty }}" data-quarter="1" placeholder="0">
                                                                </div>
                                                                <div>
                                                                    ...
                                                                    <input type="number" name="target_q2_{{ $globalKpiIndexEmpty }}" min="0" step="0.01" max="100"
                                                                           class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm kpi-target-input"
                                                                           data-kpi-index="{{ $globalKpiIndexEmpty }}" data-quarter="2" placeholder="0">
                                                                </div>
                                                                <div>
                                                                    ...
                                                                    <input type="number" name="target_q3_{{ $globalKpiIndexEmpty }}" min="0" step="0.01" max="100"
                                                                           class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm kpi-target-input"
                                                                           data-kpi-index="{{ $globalKpiIndexEmpty }}" data-quarter="3" placeholder="0">
                                                                </div>
                                                                <div>
                                                                    ...
                                                                    <input type="number" name="target_q4_{{ $globalKpiIndexEmpty }}" min="0" step="0.01" max="100"
                                                                           class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm kpi-target-input"
                                                                           data-kpi-index="{{ $globalKpiIndexEmpty }}" data-quarter="4" placeholder="0">
                                                                </div>
                                                                <div>
                                                                    ...
                                                                    <input type="number" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm bg-gray-100 sm:text-sm kpi-target-total-{{ $globalKpiIndexEmpty }}"
                                            ...
                                            <div class="kpi-entries-footer mt-1 flex flex-wrap items-center gap-2">
                                                <button type="button" class="remove-kpi inline-flex items-center px-3 py-2 border border-red-300 shadow-sm text-sm leading-4 font-medium rounded-md text-red-700 bg-white hover:bg-red-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500" style="{{ (isset($kraData['kpis']) && count($kraData['kpis']) > 1) ? '' : 'display: none;' }}" title="Remove last KPI in this KRA" aria-label="Delete KPI">Delete KPI</button>
                                                <button type="button" class="add-kpi-btn inline-flex items-center px-3 py-2 border border-gray-300 shadow-sm text-sm leading-4 font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500" data-kra-index="{{ $kraIndex }}">
                                ...
                                <div class="kra-item mb-6 p-4 border-2 border-gray-300 rounded-lg bg-gray-50" data-kra-index="0">
                                    <div class="flex justify-between items-center mb-4">
                                        @php
                                            $sgNumber = str_replace('SG', '', $form->sg_code ?? 'SG1');
                                        @endphp
                                        <h4 class="text-md font-semibold text-gray-800 kra-title">KRA {{ $sgNumber }}.1</h4>
    ...
    <script>
        // JavaScript to handle multiple KRAs, KPIs, and target values
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (addKraBtn) {
                addKraBtn.addEventListener('click', function() {
                    ...
                    const sgCode = document.getElementById('sg_code')?.value || '{{ $form->sg_code ?? "SG1" }}';
                    ...
                    kraItem.innerHTML = `
                        ...
                        <div class="kpi-container" data-kra-index="${newKraIndex}">
                            <div class="kpi-item mb-3 p-3 border border-gray-200 rounded-md bg-white">
                                ...
                                <div class="border-t border-gray-200 pt-3">
                                    <label class="block text-sm font-medium text-gray-700 mb-2">Target Values</label>
                                    <div class="grid grid-cols-2 md:grid-cols-5 gap-3">
                                        <div>
                                            <label class="block text-xs font-medium text-gray-600 mb-1">Q1</label>
                                            <input type="number" name="target_q1_0" min="0" step="0.01" 
                                                   class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm kpi-target-input" 
                                                   data-kpi-index="0" data-quarter="1" placeholder="0">
                                        </div>
                                        <div>
                                            <label class="block text-xs font-medium text-gray-600 mb-1">Q2</label>
                                            <input type="number" name="target_q2_0" min="0" step="0.01" 
                                                   class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm kpi-target-input" 
                                                   data-kpi-index="0" data-quarter="2" placeholder="0">
                                        </div>
                                        <div>
                                            <label class="block text-xs font-medium text-gray-600 mb-1">Q3</label>
                                            <input type="number" name="target_q3_0" min="0" step="0.01" 
                                                   class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm kpi-target-input" 
                                                   data-kpi-index="0" data-quarter="3" placeholder="0">
                                        </div>
                                        <div>
                                            <label class="block text-xs font-medium text-gray-600 mb-1">Q4</label>
                                            <input type="number" name="target_q4_0" min="0" step="0.01" 
                                                   class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm kpi-target-input" 
                                                   data-kpi-index="0" data-quarter="4" placeholder="0">
                                        </div>
                                        <div>
                ...
                `;
                    ...
                    updateTargetValueIndices(); // Update target value indices
                    initializeTargetValueListeners(); // Initialize listeners
                ...
                kraContainer.addEventListener('click', function(e) {
                    if (e.target.closest('.add-kpi-btn')) {
                        ...
                        allKraItems.forEach((kra, idx) => {
                            const kraIdx = parseInt(kra.getAttribute('data-kra-index'));
                            if (kraIdx < parseInt(kraIndex)) {
                        ...
                        updateTargetValueIndices(); // Update target value indices
                        attachTargetValueListeners(kpiItem); // Attach listeners to new KPI
                    }
                });
            }
            ...
            // Initialize
            updateKraRemoveButtons();
            const kpiContainers = kraContainer.querySelectorAll('.kpi-container');
            kpiContainers.forEach(container => {
                updateKpiRemoveButtons(container);
            });
            ...
            // Validate Responsible Unit and Level on submit
            const editForm = kraContainer.closest('form');
            if (editForm) {
                editForm.addEventListener('submit', function(e) {
                    ...
                    if (hasError) {
                        e.preventDefault();
                        if (window.showAlert) window.showAlert({ title: 'Notice', message: 'Please select a level (CL or UL) for each KPI.' });
                        else alert('Please select a level (CL or UL) for each KPI.');
                        return false;
                    }
                    ...
                    for (const inp of ruInputs) {
                        if (!inp.value || !inp.value.trim()) {
                            e.preventDefault();
                            if (window.showAlert) window.showAlert({ title: 'Notice', message: 'Please select at least one Responsible Unit for each KPI.' });
                            else alert('Please select at least one Responsible Unit for each KPI.');
                            return false;
                        }
                    }
                });
